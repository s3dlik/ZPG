...
LambertShader::LambertShader(Camera* camera, const char* vertexFile, const char* fragmentFile) : AbstractShader(camera, vertexFile, fragmentFile) {}

void LambertShader::activateSh(glm::mat4 M, glm::vec4 color)
{
	glm::vec3 lightPos[4];
...
void LambertShader::activate()
{
	glUseProgram(shaderProgram);
}

void LambertShader::activateFlashLight(int mod)
{
